...
from backend.db.postgres import Lead, SMB, async_session
...
FAQ_ENTRIES = [
    ...
    },
    {
        ...
        "answer": "Business loan rates vary by credit profile and loan type. Current SBA loans start at Prime + 2.75%. I can connect you with a banker for a personalized quote.",
    },
    {
...
async def search_faq(query: str) -> dict[str, Any]:
    ...
    return FAQResult(
        question=query,
        answer="I don't have a specific answer for that. Would you like me to connect you with your banker?",
        confidence=0.0,
    ).model_dump()


async def escalate_to_banker(
    smb_id: str, reason: str, urgency: str = "medium"
) -> dict[str, Any]:
    ...
    logger.info("Escalation created: lead_id=%s smb_id=%s urgency=%s", lead.id, smb_id, urgency)
